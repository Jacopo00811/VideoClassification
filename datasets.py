...
class FrameVideoDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir='ufc10', split='train', transform=None, stack_frames=True):
        ...
        self.video_paths = sorted(glob(os.path.join(root_dir, 'frames', split, '*', '*')))  # Path to frame folders
    ...
    def _get_meta(self, attr, value):
        matches = self.df[self.df[attr] == value]
        if len(matches) == 0:
            raise ValueError(f"No metadata found for {attr}={value}")
        return matches.iloc[0]['label']  # Return just the label value directly

    def __getitem__(self, idx):
        frame_dir = self.video_paths[idx]
        video_name = os.path.basename(frame_dir)  # Get the video name from the frame directory
        label = self._get_meta('video_name', video_name)

        # Load frames directly from the frame directory
        video_frames = self.load_frames(frame_dir)
